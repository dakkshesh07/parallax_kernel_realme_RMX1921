...
#ifdef CONFIG_MACH_REALME//Fanhong.Kong@ProDrv.CHG,add 2018/06/02 for SVOOC OTG
		if(chip->vbatt_num == 2)
		{
...
rerun_work:
	...
	queue_delayed_work(system_power_efficient_wq, &chg->chg_monitor_work, OPPO_CHG_MONITOR_INTERVAL);
}
#endif /* CONFIG_MACH_REALME */
...
#ifdef CONFIG_MACH_REALME//Fanhong.Kong@ProDrv.CHG,add 2018/04/20 for SVOOC
...
#ifdef CONFIG_MACH_REALME /* wangsen@BSP.POWER.Basic 2019/11/07 add for pd charge */
...
static bool smbchg_get_pd_type(void)
{
    ...
    if (likely(!chip->pmic_spmi.smb2_chip->chg.pd_active)) {
