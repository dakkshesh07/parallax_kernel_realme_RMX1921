...
static int oplus_nfc_probe(struct platform_device *pdev)
{
	...
	pr_err("%s, prop_project_operator to be read = %s", __func__,
	       prop_project_operator);
	readRet = of_property_read_string(dev->of_node, prop_project_operator,
					  &chipset_node);
	if (readRet != 0) {
		sprintf(prop_project, "chipset-%d", project);
		pr_err("%s, prop_project to be read = %s", __func__,
		       prop_project);
		readRet = of_property_read_string(dev->of_node, prop_project,
						  &chipset_node);
	}
	np = dev->of_node;
	if (readRet != 0) {
